...
def train_model(num_epochs: int = 100, batch_size: int = 8):
    ...
    bias_prob=0.5 is passed to generate_trace_pair so that roughly half of
    all generated pairs have non-zero IFL, preventing the model from
    collapsing to always predicting zero.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    ...
    # log_scale=False: raw MSE on cycle counts (0, 9, 18, ...) is more
    # transparent and avoids the relu+log1p collapse when targets are all zero.
    loss_fn    = DegradationLoss(log_scale=False)
    ...
    for epoch in range(num_epochs):
        ...
        for _ in range(batch_size):
            # bias_prob=0.5: ~57% of pairs will have ifl > 0, keeping
            # the batch from being overwhelmed by zero-target samples.
            t1, t2 = generate_trace_pair(bias_prob=0.5)
